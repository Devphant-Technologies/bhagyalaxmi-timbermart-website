...
export type ProductData = {
  ...
  overview: string[]; types: { title: string; text: string; image?: string }[];
  ...
  typesImage?: { src: string; alt: string };
};
...
export default function ProductDetail({ product }: { product: ProductData }) {
  ...
  return <main className="product-page">
    ...
    <section className="product-types"><div className="product-types-heading"><p className="eyebrow light"><span /> {product.typesEyebrow ?? "Available constructions"}</p><h2>{typesTitle[0]}<br />{typesTitle[1]}</h2>{product.typesImage&&<figure className="product-types-visual"><img src={product.typesImage.src} alt={product.typesImage.alt} /></figure>}</div><div className="product-type-grid">{product.types.map((type,i)=><article key={type.title}><span>{String(i+1).padStart(2,"0")}</span>{type.image&&<img className="species-thumb" src={type.image} alt={`${type.title} timber sample`} />}<h3>{type.title}</h3><p>{type.text}</p></article>)}</div></section>
    ...
    <footer className="contact-footer"><div><img src="/btm-logo-light.png" alt="Bhagyalaxmi Timber Mart" /><p>Complete wooden packaging solutions<br />for export & domestic use.</p></div><a href="/#products">All products <span>↗</span></a><span>© 2026 Bhagyalaxmi Timber Mart</span><p className="footer-credit">Designed and developed by <a href="https://www.instagram.com/bigartstudio?utm_source=qr&igsi=bDFhem1yc29leTRn" target="_blank" rel="noreferrer">BigArts Studio</a></p></footer>
  </main>;
}
